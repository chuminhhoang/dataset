...
class MOTRv2:
    def __init__(self, cfg) -> None:
        self.cfg = cfg
        self.dict_bboxes =  defaultdict(list)
    ...
    def preprocess(self, image, proposals): # input is bgr image
        ...
        """
        # cur_img = image[..., ::-1]
        cur_img = cur_img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        im_h, im_w = cur_img.shape[:2]

        proposals_ = []
        for proposal in proposals:
            x1, y1, x2, y2, conf, _ = proposal
            proposals_.append([(x1 + x2) / 2 / im_w,
                               (y1 + y2) / 2 / im_h,
                                (x2 - x1) / im_w, 
                                (y2 - y1) / im_h, 
                                conf])
        ...
        if max(im_h, im_w) * scale > self.input_img_width:
            scale = self.input_img_width / max(im_h, im_w)
        ...
        return img, image, torch.as_tensor(proposals_).reshape(-1, 5).to(torch.float32)
    ...
    def detect(self, image, proposals, prob_threshold=0.6, area_threshold=100):
        ...
        for xyxy, conf, identity in zip(bbox_xyxy, confs, identities):
            ...
            self.dict_bboxes[str(identity)].append([x1, y1, x2, y2])
