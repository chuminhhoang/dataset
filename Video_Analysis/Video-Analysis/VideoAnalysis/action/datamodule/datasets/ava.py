...
from VideoAnalysis.utils.tool import add_datatrain_feature
...
class AVADataset(torch.utils.data.Dataset):
    def __init__(self, cfg, train=True):
        ...
        self.max_tokens = self.cfg.max_people
        ...
        cfg_analyze = default._C.clone()
        self.video_analyze = VideoAnalyzer(cfg_analyze)

        self.dataset_roots        = {
            # heira
            "kinetics_train"  : ["",    "data/kinetics_train/", self.cfg.kinetics.sampling_factor],
            "ava_train"       : ["ava", "data/ava_train/",      self.cfg.ava.sampling_factor],
            "ava_val"         : ["ava", "data/ava_val/",        1], 
        }
        if self.train:
            self.list_of_datasets = cfg.train_dataset.split(",")
            
            for dataset in self.list_of_datasets:
                LOGGER.info(self.dataset_roots[dataset][0])
            ...
            self.list_of_datasets = cfg.test_dataset.split(",")
            for dataset in self.list_of_datasets:
                self.get_dataset(root_dir=self.dataset_roots[dataset][1], 
                                filter_seq=self.cfg.test_class, 
                                min_track_length=1, 
                                total_num_tracks=None)
            ...
            self.data = task_divider(self.data, self.cfg.test_batch_id, self.cfg.number_of_processes)
            self.track2video = task_divider(self.track2video, self.cfg.test_batch_id, self.cfg.number_of_processes)
        ...
        LOGGER.info("Number of tracks: {}".format(len(self.data)))
                ...
                if(int(video_.split("_")[-1][:-4])>min_track_length):
                    ...
                    if(total_num_tracks is not None):
                        if(count>=total_num_tracks):
                            break
        LOGGER.info("Total number of tracks: {}".format(count))
        LOGGER.info("Total number of frames: {}".format(count_f))
    ...
    def get_start_end_frame(self, list_of_frames, f_):
        ...
        if(self.train):
            ...
            start_frame  = 0
            end_frame    = len(list_of_frames) if(self.cfg.full_seq_render) else min(len(list_of_frames), self.frame_length)
            key_frame    = (start_frame+end_frame)//2
    ...
    def read_from_phalp_fast(self, idx):
        ...
        if(self.cfg.frame_rate_range>1 and self.train):

            frame_rate     = np.random.randint(1, self.cfg.frame_rate_range)
    ...
    def __getitem__(self, idx):
        ...
        if(self.train): frame_length_ = self.cfg.frame_length
        else:           frame_length_ = max(end_frame - start_frame, self.cfg.frame_length)
        ...
        # add kinetics labels
        if("kinetics" in self.cfg.action_space and not("ava" in video_name)):
        ...
        ava_pseudo_vectors_ = np.zeros((end_frame-start_frame, 1, self.cfg.extra_feat.apperance.dim))
        ...
        # self.ava_valid_classes = np.arange(1, 61)
        if(self.cfg.ava.predict_valid):
            action_label_ava_ = np.zeros((end_frame-start_frame, 1, self.cfg.ava.num_action_classes))
            action_label_ava_[:, :, :self.cfg.ava.num_valid_action_classes] = TMP_[:, :, self.ava_valid_classes-1]
        ...
        if("joints_3D" in input_data.keys()):
            ...
            # pred_cam = detection_data["camera"][start_frame:end_frame, 0]
            # rotation = np.eye(3)[None].repeat(BS, axis=0)
            # camera_center = np.zeros(shape=(BS, 2))
            # focal_length = np.full(shape=(BS, 2), fill_value=self.cfg.focal_length) 
            # img_size = np.array(detection_data["frame_size"])
            
            # joints_2d_ = perspective_projection(joints_,  # 45 points
            #                                     rotation, 
        ...
        if(self.max_tokens>1):
            ...
            for ot in range(len(other_tracks)):
                ...
                if(self.cfg.loss_on_others_action):
                ...
                other_ava_pseudo_vectors_ = np.zeros((other_end_frame-other_start_frame, 1, self.cfg.extra_feat.apperance.dim))
                ...
                if(self.cfg.ava.predict_valid):
                    action_label_ava_ = np.zeros((other_end_frame-other_start_frame, 1, self.cfg.ava.num_action_classes))
                ...
                if("joints_3D" in input_data.keys()):
                    ...
                    joints_ = joints_ + camera_
                    if(not(self.cfg.loss_on_others_action)):
        ...
        # Todo: function from tool.py to insert data in dataloader
        frame_idx = start_frame
        # other_frame_idx = other_start_frame
        root_img_folder = 'data_image'
        folder_img = os.listdir(root_img_folder)
        folder_img.sort()
        for name in folder_img:
            if frame_idx >= 65:
                break
            img_path = root_img_folder+'/'+name
            # # a = check(other_ava_pseudo_vectors_, detection_data['frame_bbox'][start_frame:65])
            # input_data['joints_2D'][frame_idx:frame_idx+1, 0, :] = add_datatrain_feature(img_path, detection_data["frame_bbox"][frame_idx:frame_idx+1], frame_idx+1, self.video_analyze)
            import cv2
            frame = cv2.imread(img_path)
            a = self.video_analyze.get_context_feature_train_version(frame, idx=frame_idx, bbox = detection_data['frame_bbox'][frame_idx:frame_idx+1])
            frame_idx += 1
        del detection_data
