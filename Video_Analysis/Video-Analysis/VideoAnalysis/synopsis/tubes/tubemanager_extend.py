...
class TubeManager:
    ...
    def __init__(self, cfg):
        ...
        for tube_idx in range(self.num_tubes):
            ...
            self.tubes[tube_idx].time_start = tube_idx*70#int(tube_idx / self.num_tubes * self.synopsis_video_length + self.synopsis_begin_frame) 
    ...
    def __compute_boxes_union(self, box1, box2, ratio_size1, ratio_size2):
        x1min, y1min, w1, h1 = box1
        x2min, y2min, w2, h2 = box2
    ...
    def __compute_area_cover_energy(self, synopsis_frame_idx):
        """Compute area cover energy
        Objects appear in a frame cover the largest area
        """
        for tube_idx in range(self.num_tubes):
            if (self.tubes[tube_idx].time_start < synopsis_frame_idx and \
                self.tubes[tube_idx].time_end > synopsis_frame_idx):
                 pass     
